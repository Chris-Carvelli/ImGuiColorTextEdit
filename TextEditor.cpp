	...
	if (!mLines.empty())
	{
		while (lineNo <= lineMax)
		{
			...
			if (errorIt != mErrorMarkers.end())
			{
				...
				{
					ImGui::BeginTooltip();
					ImGui::PushStyleColor(ImGuiCol_Text, mPalette[(int)PaletteIndex::ErrorMarkerTitleColor]);
					ImGui::Text("Error at line %d:", errorIt->first);
					ImGui::PopStyleColor();
					ImGui::Separator();
					ImGui::PushStyleColor(ImGuiCol_Text, mPalette[(int)PaletteIndex::ErrorMarkerBodyColor]);
...
const TextEditor::Palette & TextEditor::GetDarkPalette()
{
	static Palette p = { {
								 ...
								 0xffffffff, // ErrorMarkerBodyColor
						 } };
	return p;
}
